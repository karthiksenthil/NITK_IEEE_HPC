...
int Search(Board *parB,const int alpha, const int beta, int depth, int ply,
  ...
  // parallel purpose:
  Board varB = (*parB); // copying the board
  Board* B = &varB;
  // parallel purpose:
  BOOL IsCancelThreads = FALSE, IsResultFound = FALSE;
...
//{
//fprintf(stdout, "before parallel: %d\n", omp_in_parallel());
//}
  if (omp_in_parallel() == 0)
  {
#pragma omp parallel sections shared(IsCancelThreads, IsResultFound, ResultFromThread,  Full) private(B) firstprivate(/*Full,*/ m, bestmove, score, best, newfifty, p, pto, LegalMoves, gchk, newdepth, Extensions, excess, SEEScore, IsCapTopMove, U, Futile, IsPV, talpha, tbeta, varB, Moveno, NMoves) default(shared) /*schedule(runtime) */ /*if (oip==0)*/ 
  {
//#pragma omp critical (test)
//{
//fprintf(stdout, "before section: %d\n", oip);
//}
#pragma omp section 
  {
//#pragma omp critical (test2)
//{
//fprintf(stdout, "in section: %d\n", omp_in_parallel());
//}
	B = &varB; // parallel purpose
  for (Moveno = 0 ; Moveno < NMoves/2 ; Moveno++) {
//	B = &varB; // parallel purpose
//#pragma omp critical (test)
//{
//fprintf(stdout, "%d\n", IsCancelThreads);
...
//}
    ...
    if (gchk && InCM(B,gchk)) {
      UndoMove(B,m,U);
	#pragma omp critical (IOoperationfd)
	{
      ...
      SortNodes++;
      if (Post && ply==0) {
	#pragma omp critical (IOoperationfds)
	{
...
{ 
  ...
  if (CheckKey != B->Key) {fprintf(stderr,"Warning - Key Corrupted in Search() iteration step, ply=%d, depth=%d\n",ply,depth);PrintBoard(*B);while (1);}
      ...
      if (best>talpha) {
        ...
        if (score>=tbeta) {
          ...
          BestMoveRet = bestmove;
	        SortNodes++;
	#pragma omp critical (hashUpdate1)
	{
          HashUpdate(B,score,bestmove,depth,HASH_LOWER,threat,ply);
			...
			// parallel critical region: workaround a return clause
			#pragma omp critical (ReturnBestResultFromThread)
			{
				if (!IsResultFound) {
					ResultFromThread = best;
					IsResultFound = TRUE;
				}
				IsCancelThreads = TRUE;
			} //includes implicit flush
	...
	#pragma omp critical (hashUpdate)
	{
          ...
          if (Post){
	    #pragma omp critical (IOoperation1)
            {
	...
	#pragma omp critical (hashupdate0)
	{
        HashUpdate(B,score,bestmove,depth,HASH_UPPER,threat,ply);
	}
        if (Post){
          #pragma omp critical (IOoperation)
           { 
          PrintThinking(score,B);
          }
        }
...
#pragma omp critical (periodiccheck)
{
...
#pragma omp section
  {
	B = &varB; // parallel purpose
  for (Moveno = NMoves/2 ; Moveno <NMoves ; Moveno++) {
//	B = &varB; // parallel purpose
//#pragma omp critical (test)
//{
//fprintf(stdout, "%d\n", IsCancelThreads);
...
#pragma omp critical (PeriodicChecks)
{
    /* Check for time problems and new input periodically */
    if (!AbortFlag && ((Nodecount&INPUT_PERIOD)==0 || ply<4) && !bRushed) {
      if (GlobalDepth>Params.Depth && !AnalyseMode) AbortFlag = CheckTime(ply,score);
  ...
  //parallel workaround of return clause
  if (IsResultFound)
...
#pragma omp critical (hashupdates)
{
  if (!AbortFlag) {
...
#pragma omp critical (IOoperationlast)
{
